...
void VoronoiDiagram::create_mpi_handles(MPI_Datatype *MPI_POINT, MPI_Datatype *MPI_BALL, MPI_Op *MPI_ARGMAX)
{
    if (MPI_POINT)
    {
...
void VoronoiDiagram::build_random_diagram(Index m)
{
    ...
    create_mpi_handles(&MPI_POINT, &MPI_BALL, &MPI_ARGMAX);
...
void VoronoiDiagram::build_greedy_permutation(Index m)
{
    int myrank, nprocs;
    MPI_Comm_rank(comm, &myrank);
    MPI_Comm_size(comm, &nprocs);
    ...
    sites.resize(m);
    site_points.resize(m);
    my_cell_sizes.resize(m, 0), cell_sizes.resize(m);
...
{
    reptree.assign_points(site_points.begin(), site_points.end());
    reptree.build(covering_factor, leaf_size);
}
